...
public class DrawingSurfaceView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public void update(){
        ...
        // for all balls registered
        for (Ball balls: ballSet) {
            balls.dx /= 1.1; // slow down on default!
            balls.dy /= 1.1;

            balls.cx += balls.dx; //move
            balls.cy += balls.dy;

            //hit detection and teleport the balls to the right place
            if (balls.cx - balls.radius > viewWidth) { //left bound
                balls.cx = 0;
            } else if (balls.cx + balls.radius < 0) { //right bound
                balls.cx = viewWidth;
            } else if (balls.cy - balls.radius > viewHeight) { //bottom bound
                balls.cy = 0;
            } else if (balls.cy + balls.radius < 0) { //top bound
                balls.cy = viewHeight ;
            }
        }
    }
    ...
    public void render(Canvas canvas){
        ...
        Set<Ball> copy = new HashSet<>(ballSet); // prevent concurrency exception error

        for (Ball balls : copy) {
            canvas.drawCircle(balls.cx, balls.cy - balls.radius, balls.radius, balls.color);
        }
    }
